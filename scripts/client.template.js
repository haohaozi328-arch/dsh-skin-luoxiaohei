...
/* 选择器锚在 body[data-dsh-luoxiaohei]（apply 时写入的租约属性）：DSH 的
   design-platform.css 等主题样式在插件样式之后注入，同为 body 选择器时后者胜出，
   token 会被静默覆盖（背景、玻璃全部失效）。提高特异度即可压过，且不依赖注入顺序。 */
html body[data-dsh-luoxiaohei] {
...
/* ═══ M1 · 深色「深夜森林」 ═══ */
html body[data-dsh-luoxiaohei][data-ds-dark-theme] {
...
.dsh-lxh-cursor-setting .t img,
.dsh-lxh-bg-setting .t img {
  width: 18px;
  height: 18px;
  margin-right: 8px;
  object-fit: contain;
...
/* 锚点用 role="dialog"（窗口本身即面板）——旧写法 [role="dialog"].bi44ca_panel 依赖
   CSS Modules 构建 hash，换版本即失配。面板自身的 .xxxxx_panel{background:var(...)}
   与 [role="dialog"] 同特异度且后注入，故这里抬到 body[data-dsh-luoxiaohei] 之下。 */
html body[data-dsh-luoxiaohei] [role="dialog"] {
...
html body[data-dsh-luoxiaohei][data-ds-dark-theme] [role="dialog"] {
...
/* AI 回答正文：markdown 正文块本身加玻璃框。
   旧写法 [class*="gshEtW_body"] > [class*="_markdown_"] 依赖 AssistantMarkdown 的构建 hash，
   换版本即失配（该 hash 现已变为 kshsua_body）；改为直接命中 markdown 块，并用
   :not(:where([data-disclosure-row] *)) 排除工具/Think 块内部的 markdown。 */
[class*="_markdown_"]:not(:where([data-disclosure-row] *)) {
  width: fit-content;
  max-width: 100%;
  background-color: rgba(250, 246, 236, 0.5);
...
html body[data-dsh-luoxiaohei][data-ds-dark-theme] [class*="_markdown_"]:not(:where([data-disclosure-row] *)) {
...
`;
		...
		function apply(ctx) {
			...
			/* ── 设置面板内容容器 ──
			   旧写法 `[role="dialog"] .bi44ca_content` 绑定 CSS Modules 构建 hash，换版本即失配
			   （本版内容区是 y7bFDa_content / 选项区 y7bFDa_options），两行设置因此整块消失。
			   改为按结构找：对话框内的选项滚动区 → 内容区 → 对话框本身。 */
			function settingsContent() {
				const dialog = document.querySelector('[role="dialog"]');
				if (!dialog) return null;
				return dialog.querySelector('[class*="_options"]')
					|| dialog.querySelector('[class*="_content"]')
					|| dialog;
			}
			...
			function injectCursorSetting() {
				...
				row.innerHTML = '<div class="t"><img src="' + (ASSETS['thumb.png'] || '') + '" alt="" aria-hidden="true">鼠标样式</div><div class="d">选择全局鼠标光标：系统默认，或罗小黑竖大拇指（悬停可点元素时放大）</div><div class="btns"><button type="button" data-cursor="default">默认</button><button type="button" data-cursor="lxh">小黑拇指</button></div>';
			...
			function injectBgSetting() {
				...
				row.innerHTML = '<div class="t"><img src="' + (ASSETS['scissor-last.png'] || ASSETS['scissor.gif'] || '') + '" alt="" aria-hidden="true">背景图片</div><div class="d">上传本地图片作为主界面背景（自动压缩保存到本地）</div><div class="btns"><button type="button" data-bg="upload">上传图片</button><button type="button" data-bg="reset">恢复默认</button></div>';
...
/* ── M1 · 注入样式 ── */
			...
			/* 标题在中文界面是本地化文案（思考 / 读取 / 写入 / 运行命令 / 读取图片…），英文界面是
			   工具名（Think / Read / Write / Search…），所以每个图标登记多语言关键词：
			   先整体相等，再子串包含。旧写法只认 Latin think/read/write/search + 「上下文注入」，
			   界面切到中文后整排工具图标全部失配。 */
			const TOOL_ICONS = [
				{ keys: ['上下文注入', 'context injection', 'context-injection', 'skill catalog', 'skill-catalog'], a: 'tool-ctx.gif', s: 'tool-ctx.png', ms: 2100 },
				{ keys: ['think', '思考', '推理', 'reasoning'], a: 'tool-think.gif', s: 'tool-think.png', ms: 1800 },
				{ keys: ['read', '读取', '读取图片', '查看', 'read image', 'read file'], a: 'tool-read.gif', s: 'tool-read.png', ms: 3000 },
				{ keys: ['write', 'edit', '写入', '编辑', '修改', '创建', 'write file', 'edit file'], a: 'tool-write.gif', s: 'tool-write.png', ms: 900 },
			...
			function matchTool(name) {
				...
				for (let i = 0; i < TOOL_ICONS.length; i++) {
					...
					for (let k = 0; k < c.keys.length; k++) {
						if (raw === c.keys[k] || lower === c.keys[k].toLowerCase()) return c;
					}
				}
				for (let i = 0; i < TOOL_ICONS.length; i++) {
					const c = TOOL_ICONS[i];
